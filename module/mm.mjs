...
function addJabImportButtonToActorDirectory(setting) {
  ...
  $("section#actors footer.action-buttons").append(`<button class='import-simple' ${addHtml}>${"Import Character from Jab's Builds"}</button>`);
  ...
  $("section#actors footer.action-buttons button.import-simple").on( "click", async function() {
    ...
    let html = ``;
      ...
      html+=`<label for ="import-simple-text" ${addHtml}>Enter plain text from  one of Jab's builds found at </label><a href="https://www.echoesofthemultiverse.com/viewtopic.php?t=8">Jab's builds</a>
    ...
    let d = new Dialog({
      title: "Import all downloaded JSON files in folder",
      content:html,
      buttons: {
      /* commented out as we don't want to open up web site crawling without the website's permission
        ...
        two:{
          label: "Import Text From Jab's Build",
          callback: async (html) => {
