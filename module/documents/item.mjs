...
export class MM3Item extends Item {
    ...
    _preparePouvoirData(itemData) {
        ...
        for(let dmg in attaque.damage) {
            let atk = attaque.damage[dmg];
            let effects = atk.effects;
            atk.listEtat = CONFIG.statusEffects.filter(status => !effects.includes(status.id));
        }
